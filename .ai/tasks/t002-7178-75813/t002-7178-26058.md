...
**ID:** t002-7178-26058       **Status:** planned
...
(e.g. `t002-7178-26058`), point `LAAW/tools/.epoch` at
...
- Parent task [t002-7178-75813](t002-7178-75813.md) — the LAAW revamp.
  This subtask has no depends-on; the `workflow.md` rewrite
  (t002-7178-76890) and the sync scripts (t002-7190-44432) depend on
  it.
...
- Format authority: the [revamp design](../../workbench/revamp-design.md)
  ...
  binding shape.
- Decision [c037-1650-68133](../../context/c037-1650-68133.md) settles
...
6. Batch safety (`--count N`): all N share the same minute part;
   re-draw the random part on any within-batch duplicate so the batch
   always yields N unique ids.
7. Directory safety (`--dir PATH`): PATH must exist (error if not);
   after minting, if an id's stem already exists in PATH as a file
   *or* a directory, re-draw its random part and re-check; combine
   with the within-batch rule so batch + directory are both clean.
8. Mint loop, as guidance (not a script):
   ...
   for each of the N requested ids:
     loop:
       ...
       if --dir given and candidate exists in dir as file or folder: re-draw
     accept candidate
   ```
...
- `python3 tools/generate-id.py --count 5` → 5 lines, all sharing the
  same 3+4 minute part, all 5 ids unique
...
- `--dir` collision: in a scratch dir, touch a file named
  `t{just-minted-id}.md`, re-run with `--dir` at it, and confirm the
  returned stem is not the existing one
...
- Emitted ids look byte-identical in structure to already-minted
  workspace ids (`t002-7178-26058`, `c037-1650-68133`) — in
